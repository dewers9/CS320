...
let apply_cycle (funcs : ('a -> 'a) list) (n : int) (x : 'a) : 'a =
  let rec helper (rem_funcs : ('a -> 'a) list) (n : int) (x : 'a) (all_funcs : ('a -> 'a) list) : 'a =
    if n <= 0 then 
      x
    else
      (match rem_funcs with
      | [] -> helper (all_funcs) (n) (x) (all_funcs)
      | h :: t -> helper (t) (n-1) (h x) (all_funcs))
  in
  helper (funcs) (n) (x) (funcs)
