...
let rec get_path (len : int) (p : ('a -> 'a)) (ps : 'a) : 'a list =
  match len with
  | 0 -> [ps]
  | len -> begin
    ps :: get_path (len-1) (p) (p (ps)) 
  end

let rec check_path (l : 'a list) (g : 'a -> 'a -> bool) : bool = 
  match l with
  | [] -> true
  | fst :: snd :: t -> (g fst snd) && (check_path (snd :: t) g)
  | fst :: t -> true
...
let rec walks
  ...
  if len < 0 then
    ...
    | [] -> []
    | (p, ps) :: t -> (
      let complete_path = get_path len p ps in
      if check_path complete_path g then
        last_elem complete_path :: walks g len t
      else
        walks g len t
    )
    | _ -> assert false
